...
from module.graph_generative_modeling import Discriminator, DiscriminatorV2

# Max length including <bos> and <eos>
MAX_GQA_LENGTH = 20


class GQAPlainModel(nn.Module):
...
class GQAModelGIN(nn.Module):
    def __init__(self, num_answers, n_layers=2):
        ...
        self.discriminator_node = DiscriminatorV2(hidden_dim=36 * hid_dim)
        self.discriminator_edge = DiscriminatorV2(hidden_dim=36 * 36)
...
class GQAModelGAT(nn.Module):
    def __init__(self, num_answers, n_layers=1):
        super().__init__()
        # Build LXRT encoder
        self.lxrt_encoder = LXRTEncoderFeature(
            args,
