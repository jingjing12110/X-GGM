...
def load_tf_weights_in_bert(model, tf_checkpoint_path):
    ...
    try:
        ...
        print(
            "Loading a TensorFlow models in PyTorch, requires TensorFlow to "
    ...
    for name, array in zip(names, arrays):
        name = name.split('/')
        # adam_v and adam_m are variables used in AdamWeightDecayOptimizer
        # to calculated m and v
...
class BertSelfattLayer(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask):
        # Self attention attends to itself, thus keys and querys are the same
        # (input_tensor).
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, state_dict=None,
        ...
        Params:
            pretrained_model_name_or_path: either:
                - a str with the name of a pre-trained model to load selected in
                the list of:
                    . `bert-base-uncased`
                    . `bert-large-uncased`
                    . `bert-base-cased`
            ...
            state_dict: an optional state dictionnary (collections.OrderedDict
             object) to use instead of Google pre-trained models
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        if visual_attention_mask is not None:
            ...
            extended_visual_attention_mask = extended_visual_attention_mask.to(
                dtype=next(self.parameters()).dtype)  # fp16 compatibility
            extended_visual_attention_mask = (1.0 - extended_visual_attention_mask
                                              ) * -10000.0
        else:
...
class LXRTPretraining(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                ...
                visual_feats=None, pos=None, obj_labels=None, matched_label=None,
                ans=None):
        ...
        if ans is not None and self.task_qa:
            ...
            # Previous: (loss *0) for 6 epochs, (loss *2) for 6 epochs.
            # (Used 10 instead of 6 in EMNLP paper)
