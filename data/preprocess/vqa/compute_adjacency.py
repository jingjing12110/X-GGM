...
def main():
    ...
    obj_h5 = h5py.File(os.path.join(
        'data/mscoco_imgfeat/', f'{mode}_obj36.h5'), 'r')
    obj_label = read_txt(f'data/mscoco_imgfeat/objects_vocab.txt')
    attr_label = read_txt(f'data/mscoco_imgfeat/attributes_vocab.txt')
    ...
    h5_adj = h5py.File(os.path.join(
        'data/mscoco_imgfeat/', f'{mode}_obj36_adj_v2.h5'), 'w')
    ...
    for img_id in obj_h5.keys():
        ...
        # matrix = torch.mm(matrix_class, matrix_attribute.transpose(0, 1)) + \
        #     torch.mm(matrix_class, matrix_class.transpose(0, 1)) +\
        #     torch.mm(matrix_attribute, matrix_attribute.transpose(0, 1))
